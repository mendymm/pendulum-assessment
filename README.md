...
The simulation nodes are modeled as a state machine, allowing me to thoroughly test them, and fuzz them with fast-check. The transition tests treat a type-checked table (accepted / noop / rejected per status) as the single source of truth, and property-check that the real state machine agrees for any fuzzed payload — so the collision-restart commands (`haltForRestart`, `restart`, `relaunch`) are covered the same way the lifecycle commands are, and a separate suite pins down the restart *effects* (the collision report, the ack, the relaunch-from-config-angle).

### Running the tests

All the meaningful tests live in the `sim-node` package. Run them with:

```bash
npm -w @pendulum/sim-node run test
```

And type-check every workspace with:
...
- Testing the gateway's collision-restart coordinator. The node side of the barrier is well covered, but the gateway's ack-collection / timeout / membership-snapshot logic is imperative module-global code that I did not get to unit test.
- Testing the gateway's HTTP command broadcast. We would want to regression test that.
- Testing the shell non-functional parts, I.E. does a rejected command correctly result in an http response with an error?
- E2E UI testing.
- Some more I forgot to list.


Unlike the rest of the backend code, the tests are llm generated, and I review them and ensure they are correct before committing.
